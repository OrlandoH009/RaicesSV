...
(function initGameCoasters(){
  ...
  function resetGame() {
    ...
    if (gameMode === 'pvp') {
      ...
      bot.maxSpeed = 5.6 * COASTERS_SPEED_MULT;
    } else if (botDifficulty === 'medium') {
      bot.maxSpeed = 6.8 * COASTERS_SPEED_MULT;
    } else {
      bot.maxSpeed = 7.8 * COASTERS_SPEED_MULT;
    }
  ...
  function spawnEntities() {
    ...
    // Tasas de aparición subidas a pedido (la carrera se sentía poco
    // desafiante, con muy pocos baches/tráfico cruzando la calle).
    ['p1', 'p2'].forEach(side => {
      if (Math.random() < 0.008 && roadObstacleCount(side, ['bache', 'tumulo']) < 4) {
        if (Math.random() > 0.5) spawnBache(side); else spawnTumulo(side);
      }
      if (Math.random() < 0.006 && roadObstacleCount(side, ['traffic']) < 2) {
        spawnTraffic(side);
      }
...
(function initGameTorito(){
  ...
  // Velocidad y obstáculos subidos a pedido: la calle se sentía poco
  // desafiante incluso en difícil, con muy pocas carretas/baldes cruzando.
  const gameConfig = {
    easy: { baseSpeed: 4.6, energyDrainRate: 0.024, obstacleDrain: 14, turboSpeed: 7.6 },
    hard: { baseSpeed: 6.6, energyDrainRate: 0.038, obstacleDrain: 18, turboSpeed: 9.8 }
  };
  ...
  function spawnEntities() {
    ...
    lane = Math.floor(Math.random() * lanesCount);
    // Carreta obstacle
    if (!isLaneOccupied(lane) && Math.random() < (gameDifficulty === 'hard' ? 0.014 : 0.009) && countBodies('carreta') < 3) {
      spawnCarreta(lane);
    }

    lane = Math.floor(Math.random() * lanesCount);
    // Balde de agua obstacle
    if (!isLaneOccupied(lane) && Math.random() < (gameDifficulty === 'hard' ? 0.012 : 0.008) && countBodies('agua') < 3) {
      spawnAgua(lane);
    }
