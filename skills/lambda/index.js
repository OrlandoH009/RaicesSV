    ...
    function obtenerSystemPromptPlanificador(idioma) {
        const { formatted: hoyStr, year } = obtenerInfoFecha(idioma);
        if (idioma === 'en') {
            return `You are "Pupusita" in "Trip Planner" mode. ALWAYS respond in English. Create short, realistic itineraries in El Salvador using dollars ($ USD), based on the group size given by the user. Today's date is ${hoyStr}.
    CRITICAL PLAN RULES:
    1. Strict format: Zero paragraphs, zero introductory texts. Respond DIRECTLY with the numbered list.
    2. Detail level: EXACTLY 2 concrete activities/stops. Each one is a SINGLE short phrase (max 12 words) with its total cost for the whole group. No extra description.
    3. Costs: ONE single combined line, **Transporte y comida**, with the estimated total for the whole group (do not split into separate lines).
    4. Totals: End with **Total del grupo** and **Total por persona**. These MUST be mathematically correct: **Total del grupo** = the exact sum of every cost listed above (each activity's cost plus the Transporte y comida line), added digit by digit, not estimated. **Total por persona** = Total del grupo divided by the number of people, rounded to the nearest dollar. Double-check the arithmetic before answering; a wrong sum is a critical failure.
    ...
    10. Voice output: This will be read aloud by a voice assistant, so keep each activity to one short spoken sentence and avoid symbols that don't read well out loud.`;
        ...
        return `Eres "Pupusita" en modo "Planificador de salidas". Responde SIEMPRE en español. Crea itinerarios cortos y realistas en El Salvador usando dólares ($ USD), basados en la cantidad de personas indicada por el usuario. La fecha de hoy es ${hoyStr}.
    ...
    2. Nivel de detalle: EXACTAMENTE 2 actividades/paradas concretas. Cada una es UNA sola frase corta (máximo 12 palabras) con su costo total para todo el grupo. Sin descripción extra.
    3. Costos: UNA sola línea combinada, **Transporte y comida**, con el estimado total para todo el grupo (no la dividas en líneas separadas).
    4. Totales: Termina con **Total del grupo** y **Total por persona**. Deben ser matemáticamente correctos: **Total del grupo** = la suma exacta de cada costo mencionado arriba (cada actividad más la línea de Transporte y comida), sumada cifra por cifra, no estimada. **Total por persona** = Total del grupo dividido entre el número de personas, redondeado al dólar más cercano. Verifica la suma antes de responder; una suma incorrecta es una falla crítica.
    ...
    async function preguntarAPupusita(systemPrompt, messages, idioma) {
        try {
            ...
            const data = await new Promise((resolve, reject) => {
                ...
                req.on('timeout', () => {
                    req.destroy(new Error('Tiempo de espera agotado al contactar a Pupusita'));
                });
                req.on('error', (err) => {
                    reject(err);
                });
            ...
            return data || (idioma === 'en'
                ? 'I could not generate a response this time.'
                : 'No pude generar esta respuesta esta vez.');
        } catch(e) {
            console.log('Error llamando al chatbot', e);
            return idioma === 'en'
                ? 'I could not reach Pupusita right now. Please try again in a moment.'
                : 'No pude conectarme con Pupusita en este momento. Inténtalo de nuevo en un momento.';
        }
    }

    async function enviarRespuestaProgresiva(handlerInput, mensaje) {
        try {
            const { apiAccessToken, apiEndpoint } = handlerInput.requestEnvelope.context.System;
            const requestId = handlerInput.requestEnvelope.request.requestId;
            const bodyData = JSON.stringify({
                header: { requestId },
                directive: { type: 'VoicePlayer.Speak', speech: mensaje }
            });
            ...
            await new Promise((resolve) => {
                ...
                req.on('error', (err) => {
                    console.log('No se pudo enviar la respuesta progresiva', err);
                    resolve();
                });
                req.write(bodyData);
                req.end();
            ...
            console.log('No se pudo enviar la respuesta progresiva', e);
        }
    }

    async function preguntarConMasTiempo(handlerInput, idioma, systemPrompt, messages, mensajeEspera) {
        await enviarRespuestaProgresiva(handlerInput, mensajeEspera);
        return preguntarAPupusita(systemPrompt, messages, idioma);
    }
    ...
    function preguntaParaPaso(idioma, paso) {
        const preguntas = {
            actividad: {
                en: 'How exciting, let\'s build your perfect trip through El Salvador! First tell me what kind of experience you are looking for: gastronomy, history and sites, events and festivals, legends, or a bit of everything. For example, you can say "let\'s say gastronomy".',
                es: '¡Qué emoción, vamos a armar tu viaje perfecto por El Salvador! Primero cuéntame qué tipo de experiencia buscas: puede ser gastronomía, historia y sitios culturales, eventos y fiestas, leyendas, o un poco de todo. Por ejemplo, puedes decir "digamos gastronomía".'
            },
            ubicacion: {
                en: 'I love that choice! Now, from which city or town will you start your trip? For example, say "let\'s say San Salvador".',
                es: '¡Me encanta esa elección! Ahora dime, ¿desde qué ciudad o municipio vas a iniciar tu salida? Por ejemplo, di "digamos San Salvador".'
            },
            personas: {
                en: 'Great. How many people will be going on the trip? That way I can calculate transport and food costs for the whole group. For example, say "let\'s say four".',
                es: 'Genial. ¿Cuántas personas van a participar en la salida? Así calculo bien los costos de transporte y comida para el grupo. Por ejemplo, di "digamos cuatro".'
            },
            presupuesto: {
                en: 'Got it! Now let\'s talk about budget, to build a plan that fits well: are you looking for something affordable, moderate, or are you open to a higher budget? For example, say "let\'s say affordable".',
                es: '¡Anotado! Ahora hablemos de presupuesto, para armarte un plan que se ajuste bien: ¿buscas algo económico, moderado, o estás dispuesto a un presupuesto alto? Por ejemplo, di "digamos económico".'
            },
            tiempo: {
                en: 'Perfect. Now tell me how much time you have available for this trip: half a day, a full day, or the whole weekend? For example, say "let\'s say half a day".',
                es: 'Perfecto. Ahora dime cuánto tiempo tienes disponible para tu salida: ¿medio día, un día completo, o todo el fin de semana? Por ejemplo, di "digamos medio día".'
            },
            detalles: {
                en: 'One last thing: is there any specific dish, place, or preference you want to include no matter what? If not, just say "none" and I\'ll build your plan with what we have.',
                es: 'Una última cosa: ¿hay algún platillo, lugar específico o preferencia que quieras incluir sí o sí? Si no, dime simplemente "ninguna" y armo tu plan con lo que ya tenemos.'
            },
            modificar: {
                en: 'Tell me what you\'d like to change about the plan, for example swapping an activity, adjusting the budget, or the number of people, and I\'ll update it.',
                es: 'Cuéntame qué te gustaría cambiar del plan, por ejemplo cambiar una actividad, ajustar el presupuesto o el número de personas, y lo actualizo.'
            }
        };
        const entry = preguntas[paso];
        if (!entry) return null;
        return idioma === 'en' ? entry.en : entry.es;
    }
    ...
    const ModoLibreIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            if (attributes.modo === 'planificador' && attributes.paso && attributes.datosPlan && Object.keys(attributes.datosPlan).length > 0) {
                attributes.confirmandoCambioModo = true;
                handlerInput.attributesManager.setSessionAttributes(attributes);

                const speakOutput = idioma === 'en'
                    ? 'Are you sure? You still have a trip plan in progress and switching to free mode will lose it. Say yes to switch anyway, or no to keep building your plan.'
                    : '¿Seguro? Todavía tienes un plan en construcción y si cambias a modo libre se perderá. Di sí para cambiar de todos modos, o no para seguir armando tu plan.';
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
    ...
    const ConsultaLibreIntentHandler = {
        ...
        async handle(handlerInput) {
            ...
            if (attributes.modo === 'planificador' && attributes.paso) {
                return manejarRespuestaPlanificador(handlerInput, consulta);
            }
            ...
            const mensajeEspera = idioma === 'en'
                ? 'One moment...'
                : 'Un momento...';
            const reply = await preguntarConMasTiempo(handlerInput, idioma, obtenerSystemPromptLibre(idioma), [{ role: 'user', content: consulta }], mensajeEspera);
    ...
    const ModoPlanificadorIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            if (attributes.paso && attributes.datosPlan && Object.keys(attributes.datosPlan).length > 0) {
                const preguntaActual = preguntaParaPaso(idioma, attributes.paso);
                if (preguntaActual) {
                    const retomarMsg = idioma === 'en'
                        ? `Let's continue where we left off. ${preguntaActual}`
                        : `Retomamos donde lo dejamos. ${preguntaActual}`;
                    return handlerInput.responseBuilder
                        .speak(retomarMsg)
                        .reprompt(preguntaActual)
                        .getResponse();
                }
            }
            ...
            const speakOutput = preguntaParaPaso(idioma, 'actividad');
    ...
    const ModificarPlanIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            const speakOutput = preguntaParaPaso(idioma, 'modificar');
    ...
    async function manejarRespuestaPlanificador(handlerInput, texto) {
        const idioma = obtenerIdioma(handlerInput);
        const attributes = handlerInput.attributesManager.getSessionAttributes();
        ...
        if (!texto) {
            ...
            if (paso === 'actividad') {
                ...
                const speakOutput = preguntaParaPaso(idioma, 'ubicacion');
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
            ...
            if (paso === 'ubicacion') {
                ...
                const speakOutput = preguntaParaPaso(idioma, 'personas');
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
            ...
            if (paso === 'personas') {
                ...
                const speakOutput = preguntaParaPaso(idioma, 'presupuesto');
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
            ...
            if (paso === 'presupuesto') {
                ...
                const speakOutput = preguntaParaPaso(idioma, 'tiempo');
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
            ...
            if (paso === 'tiempo') {
                ...
                const speakOutput = preguntaParaPaso(idioma, 'detalles');
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
            ...
            if (paso === 'detalles') {
                ...
                const mensajeEspera = idioma === 'en'
                    ? 'Give me a moment while I put together your perfect plan...'
                    : 'Dame un momento mientras armo tu plan perfecto...';

                const userPrompt = construirPromptPlan(idioma, attributes.datosPlan);
                const reply = await preguntarConMasTiempo(handlerInput, idioma, obtenerSystemPromptPlanificador(idioma), [{ role: 'user', content: userPrompt }], mensajeEspera);
                const replyLimpio = limpiarVoz(reply);
            ...
            if (paso === 'modificar') {
                ...
                const mensajeEspera = idioma === 'en'
                    ? 'Give me a moment while I update your plan...'
                    : 'Dame un momento mientras actualizo tu plan...';
            ...
            const preguntaActual = preguntaParaPaso(idioma, paso);
            if (preguntaActual) {
                const conAviso = idioma === 'en'
                    ? `Sorry, I didn't catch that. ${preguntaActual}`
                    : `Disculpa, no logré entenderte. ${preguntaActual}`;
                return handlerInput.responseBuilder
                    .speak(conAviso)
                    .reprompt(preguntaActual)
                    .getResponse();
            ...
            const speakOutput = idioma === 'en'
                ? 'Say "plan" and I\'ll help you build your trip step by step.'
                : 'Di "planificar" y te ayudo a armar tu viaje paso a paso.';
            return handlerInput.responseBuilder
                .speak(speakOutput)
                .reprompt(speakOutput)
                .getResponse();
    ...
    const RespuestaPlanificadorIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            return manejarRespuestaPlanificador(handlerInput, texto);
        }
    };
    ...
    const yesIntentHandler = {
        ...
        handle(handlerInput) {
            ...
            if (attributes.confirmandoCambioModo) {
                attributes.confirmandoCambioModo = false;
                attributes.modo = 'libre';
                attributes.paso = null;
                attributes.datosPlan = {};
                handlerInput.attributesManager.setSessionAttributes(attributes);
                ...
                const speakOutput = idioma === 'en'
                    ? 'Alright, switching to free mode. What would you like to know about Salvadorean culture?'
                    : 'Listo, cambiamos a modo libre. ¿Qué quieres saber sobre la cultura salvadoreña?';
                return handlerInput.responseBuilder
                    .speak(speakOutput)
                    .reprompt(speakOutput)
                    .getResponse();
    ...
    const IntentReflectorHandler = {
        ...
        handle(handlerInput) {
            const idioma = obtenerIdioma(handlerInput);
            const speakOutput = idioma === 'en'
                ? "Sorry, I'm not sure how to help with that. You can say \"free mode\" to ask me something about El Salvador, or \"plan\" to build a trip."
                : 'Disculpa, no supe cómo ayudarte con eso. Puedes decir "modo libre" para preguntarme algo sobre El Salvador, o "planificar" para armar un viaje.';
    ...
    const ErrorHandler = {
        ...
        handle(handlerInput, error) {
            let idioma = 'es';
            try {
                idioma = obtenerIdioma(handlerInput);
            } catch (e) {
            }

            const speakOutput = idioma === 'en'
                ? 'I had trouble doing what you asked. Please try again.'
                : 'Tuve un problema para hacer lo que pediste. Inténtalo de nuevo.';
